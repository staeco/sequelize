...
class Model {
    ...
    static _validateIncludedElement(include, tableNames, options) {
        ...
        if (include.association.through &&
    ...
    static addScope(name, scope, options) {
        ...
        if (((name === 'defaultScope' && Object.keys(this.options.defaultScope).length > 0) || name in this.options.scopes) &&
    ...
    static async findOne(options) {
        ...
        if (options.limit === undefined) {
            ...
            if (!options.where ||
                !_.some(options.where, (value, key) => (key === this.primaryKeyAttribute || uniqueSingleColumns.includes(key)) &&
    ...
    static async findOrBuild(options) {
        if (!options || !options.where || arguments.length > 1) {
            throw new Error('Missing where attribute in the options parameter passed to findOrBuild. ' +
        ...
        const recursiveBulkCreate = async (instances, options) => {
            ...
            if (options.individualHooks) {
                ...
                if (options.include && options.include.length) {
                    await Promise.all(options.include
    ...
    async save(options) {
        ...
        if (options.fields.length && this.isNewRecord && this._options.include && this._options.include.length) {
            await Promise.all(this._options.include
                .filter(include => include.association instanceof BelongsTo)
                .map(async (include) => {
