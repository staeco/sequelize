...
class PostgresQueryInterface extends QueryInterface {
    ...
    async ensureEnums(tableName, attributes, options, model) {
        ...
        for (i = 0; i < keyLen; i++) {
            ...
            if (type instanceof DataTypes.ENUM ||
                ...
                promises.push(this.sequelize.query(sql, {
                    ...options,
                    plain: true,
                    raw: true,
    ...
    async dropEnum(enumName, options) {
        options = options || {};
        return this.sequelize.query(this.queryGenerator.pgEnumDrop(null, null, this.queryGenerator.pgEscapeAndQuote(enumName)), { ...options, raw: true });
    }
    /**
    ...
    async dropAllEnums(options) {
        ...
        return await Promise.all(enums.map(result => this.sequelize.query(this.queryGenerator.pgEnumDrop(null, null, this.queryGenerator.pgEscapeAndQuote(result.enum_name)), { ...options, raw: true })));
    }
    /**
