...
class QueryGenerator {
    ...
    getConstraintSnippet(tableName, options) {
        ...
        switch (options.type.toUpperCase()) {
            ...
            case 'FOREIGN KEY':
                ...
                const quotedReferences = typeof references.field !== 'undefined'
            ...
            collection.forEach((item, index) => {
                ...
                if (typeof item === 'string') {
                    ...
                    if (index > 0 && orderIndex !== -1) {
                        ...
                        else if (previousModel.rawAttributes !== undefined &&
    ...
    */
    selectQuery(tableName, options, model) {
        ...
        if (subQuery) {
            ...
            if (options.groupedLimit) {
                ...
                mainQueryItems.push(this.selectFromTableFragment(options, mainTable.model, attributes.main, `(${options.groupedLimit.values
                    .map(value => {
                    let groupWhere;
                    if (whereKey) {
                        groupWhere = {
    ...
    generateInclude(include, parentTableName, topLevelInfo) {
        ...
        if (topLevelInfo.names.name !== parentTableName.externalAs &&
    ...
    generateJoin(include, topLevelInfo) {
        ...
        if ((topLevelInfo.options.groupedLimit && parentIsTop) ||
    ...
    generateThroughJoin(include, includeAs, parentTableName, topLevelInfo) {
        ...
        if (topLevelInfo.subQuery && !include.subQuery && include.parent.subQuery && !parentIsTop) {
            ...
            const joinSource = this._getAliasForField(tableSource, `${tableSource}.${attrSource}`, topLevelInfo.options) ||
    ...
    whereItemQuery(key, value, options = {}) {
        if (value === undefined) {
            return null;
            // TODO: this involves major api changes to catch the throw
            // throw new Error(`WHERE parameter "${key}" has invalid "undefined" value`);
        }
        if (typeof key === 'string' && key.includes('.') && options.model) {
            const keyParts = key.split('.');
            if (options.model.rawAttributes[keyParts[0]] &&
